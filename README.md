...
На экране три поля. Тип зацепки определяется без модели. Если зацепки однозначны и это три разные точки, собираются поля карт и сайта: сначала API, если поля нет — один GET открытой страницы, иначе «не найдено». Капчу не обходим. Без ключей 2ГИС/Яндекс поля карт будут пустыми; сайт всё равно пробуем открыть одним GET.
